...
namespace OpenMLTD.MilliSim.Rendering.Extensions {
    public static class Direct2DDeviceContextExtensions {
        ...
        public static void DrawImage(this DeviceContext context, D2DImage image, float destX, float destY) {
            context.DrawImage(image, destX, destY, InterpolationMode.Linear, CompositeMode.SourceOver);
        }

        public static void DrawImage(this DeviceContext context, D2DImage image, float destX, float destY, InterpolationMode interpolationMode, CompositeMode compositeMode) {
        ...
        public static void DrawImage(this DeviceContext context, D2DImage image, float destX, float destY, float srcX, float srcY, float srcWidth, float srcHeight) {
            context.DrawImage(image, destX, destY, srcX, srcY, srcWidth, srcHeight, InterpolationMode.Linear, CompositeMode.SourceOver);
        }

        public static void DrawImage(this DeviceContext context, D2DImage image, float destX, float destY, float srcX, float srcY, float srcWidth, float srcHeight, InterpolationMode interpolationMode, CompositeMode compositeMode) {
            var dstOffset = new RawVector2(destX, destY);
